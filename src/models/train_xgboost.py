...
def train_xgboost_model(args, data_path="data/processed/final_dataset.csv", models_dir="models", reports_dir="reports"):
    """
    Trains an XGBoost model with manual CV and threshold tuning per fold,
    ...
    param_grid = {
        ...
        'scale_pos_weight': [scale_pos_weight_calc, scale_pos_weight_calc * 2, scale_pos_weight_calc * 0.5],
        'subsample': [0.8, 1.0],
    ...
    for i, params in enumerate(param_combinations):
        ...
        for fold, (train_idx, val_idx) in enumerate(cv.split(X_train_full, y_train_full)):
            ...
            train_auc = results['validation_0'][eval_metric][model.best_iteration]
            val_auc = results['validation_1'][eval_metric][model.best_iteration]
            logger.info(f"Fold {fold} - Iter: {model.best_iteration}, Train {eval_metric}: {train_auc:.4f}, Val {eval_metric}: {val_auc:.4f}, Fold Best F2: {fold_best_f2:.4f}")

        avg_f2 = np.mean(fold_f2_scores)
        avg_iter = int(np.mean(fold_iterations))
        logger.info(f"Avg CV F2 Score (with tuned thresholds): {avg_f2:.4f}, Avg Best Iteration: {avg_iter}")
    ...
    # Final Threshold Tuning using OOF probabilities of the best parameters
    ...
    final_model = XGBClassifier(
        **best_params,
        n_estimators=best_avg_iteration,
        use_label_encoder=False,
        eval_metric=eval_metric,
        tree_method='hist',
        device='cuda',
    ...
    # Get metrics for positive class specifically
    ...
    # Weighted metrics for overall context
    ...
    report_data = {
        ...
        "Scoring_Metric": "f2_per_fold_threshold_random_search",
        "Eval_Metric": eval_metric,
...
def main(args=None):
    ...
    model_type = getattr(args, "model", "xgboost")
